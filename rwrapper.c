...
int main(int argc, char** argv, char** envp) {
	...
	char* seed_raw = getenv("RWRAPPER_SEED");
	if (!seed_raw) {
		fprintf(stderr, "you need to specify RWRAPPER_SEED\n");
		return 1;
	}
	char* seed_end = NULL;
	unsigned long long seed_num = strtoull(seed_raw, &seed_end, 10);
	if (!*seed_raw || *seed_end) {
		fprintf(stderr, "RWRAPPER_SEED must be a valid unsigned long long\n");
		return 1;
	}
	size_t seed_size = sizeof(unsigned long long);
	if (randombytes_SEEDBYTES < seed_size) {
		seed_size = randombytes_SEEDBYTES;
	}
	seed_t seed = { 0 };
	memcpy(seed, &seed_num, seed_size);

	debug = getenv("RWRAPPER_DEBUG");

	if (argc < 2) {
		dbg("usage: %s <command> [args...]\n", argv[0]);
		return 1;
	}

	pid_t child = fork();
	if (child == 0) {
		// We're single-threaded, no need for signal safety!
		char** args = malloc(sizeof(char*) * argc);
		args[0] = argv[1];
		for (size_t idx = 1; idx + 1 < argc; idx++) {
			args[idx] = argv[idx + 1];
		}
		args[argc - 1] = 0;
	...
	for (;;) {
		...
		if (WIFSTOPPED(wstatus)) {
			...
			if (WSTOPSIG(wstatus) == SIGTRAP|0x80) {
				// Got a syscall!
				int res = ptrace(PTRACE_GET_SYSCALL_INFO, child, sizeof(struct ptrace_syscall_info), &syscall_info);
				if (res == -1) {
				...
				switch (syscall_info.op) {
					case PTRACE_SYSCALL_INFO_ENTRY:
						...
						if (strcmp(last_syscall, "connect") == 0) {
							dbg("checking whether we need to fixup connect address...\n");
					...
					case PTRACE_SYSCALL_INFO_EXIT:
						dbg("syscall exit: %s() = %lld\n", last_syscall, syscall_info.exit.rval);
						if (strcmp(last_syscall, "getrandom") == 0) {
							dbg("faking getrandom at %p...\n", (void*) last_args[0]);
